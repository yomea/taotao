...
// "/new/helloService.ashx?m=ls&sso=0",dataType: "jsonp",success: function(b) {
...
var category = {OBJ: $("#_JD_ALLSORT"),
       ...
       /*
		 * var a, b = {}, c = $("#categorys-2013"); b.data = this.DATA_Simple,
		 * c.addClass("categorys-2014"), a = this.TPL_Simple.process(b); var d =
		 * $("#_JD_ALLSORT"); d.html(a), $.bigiframe(d)
		 */
    },FN_GetData: function() {
    	// 使用jsonp来实现跨域请求
...
// a && a.info && $("#loginbar").html(a.info), a && a.sso && $.each(a.sso,
// function() {
// $.getJSON(this);
//                
// })
...
// document.getElementById("settleup") ? (null != MCART.DATA_Amount &&
// ($("#settleup s").eq(0).addClass("shopping"),
// $("#shopping-amount").html(MCART.DATA_Amount)), $("#settleup
// dl").Jdropdown({delay: 200}, function() {
// //MCART.FN_Refresh(), $("#settleup-url").attr("href",
// "http://cart.jd.com/cart/cart.html?r=" + +new Date)
// }
//        
// )) : (null != MCART.DATA_Amount &&
// $("#shopping-amount").html(MCART.DATA_Amount), $("#settleup-2013
// dl").Jdropdown({delay: 200}, function() {
// MCART.FN_Refresh(), $("#settleup-url").attr("href",
// "http://cart.jd.com/cart/cart.html?r=" + +new Date)
// }));
...
var $o = function() {
// function a() {
// this.length = 0, this.index = -1, this.iLastModified = 0, this.lastKeyword =
// !1
// }
// var b = {};
...
// };
// var c = '<a style="color:#005AA0" onclick="$o.del(event)">\u5220\u9664</a>',
// d = "\u641c\u7d22\u5386\u53f2", e = '<li id="d_#{id}"
// suggest-pos="#{suggest_pos}" title="#{title}" onclick="$o.clickItem(this)"
// history="1"><div class="search-item"
// style="color:#005AA0">#{keyword}</div><div class="search-count">' + d +
// "</div></li>", f = '<li id="d_#{id}" suggest-pos="#{suggest_pos}"
// title="#{title}" onclick="$o.clickItem(this)"><div
// class="search-item">#{keyword}</div><div
// class="search-count">\u7ea6#{amount}\u4e2a\u5546\u54c1</div></li>', g = '<div
// id="d_#{id}" suggest-pos="#{suggest_pos}" class="#{className}"
// title="#{title}" cid="#{cid}" cLevel="#{cLevel}"
// onclick="$o.clickItem(this)"><div
// class="search-item">\u5728<strong>#{cname}</strong>\u5206\u7c7b\u4e2d\u641c\u7d22</div><div
// class="search-count">\u7ea6#{amount}\u4e2a\u5546\u54c1</div></div>#{categorys}',
// h = '<li class="fore1"><div id="d_#{id}" suggest-pos="#{suggest_pos}"
// class="fore1" title="#{title}" onclick="$o.clickItem(this)"><div
// class="search-item">#{keyword}</div><div class="search-count"
// #{style}>\u7ea6#{amount}\u4e2a\u5546\u54c1</div></div>#{categorys}</li>', i =
// "http://dd.search.jd.com/?key=#{keyword}", j = "#FFDFC6", k = "#FFF", l =
// $("#key"), m = $("#shelper");
...
// null == e || $.trim(e).isEmpty() || l.val(e), search.additinal = "&suggest="
// + a.getAttribute("suggest-pos"), this.click = a.id.substr(2) -
// m.find("li[history='1']").length, search("key")
...
// e.css("background-color", k), e.attr("history") &&
// e.find(".search-count").html(d), -1 == b.index && -1 == a && (a = 0), b.index
// = (b.length + b.index + a) % b.length;
// var f = $("#d_" + b.index);
// if (f.length > 0) {
// f.attr("history") && f.find(".search-count").html(c),
...
// var i = f.attr("cLevel");
// search.cLevel = null != i && "" != i ? i : null, search.ev_val = null,
// search.additinal = "&suggest=" + f.attr("suggest-pos"), b.click = b.index -
// m.find("li[history='1']").length
// }
...
// $.ajax({url: d,dataType: "jsonp",scriptCharset: "utf-8",jsonp:
...
// c.init();
// var d = "", i = 0;
// window.pageConfig && window.pageConfig.searchType && (i =
// window.pageConfig.searchType);
// for (var j = 0, k = "", n = !1, o = 0, p = $.trim(l.val()), q = 0, r =
// a.length; r > q; q++) {
// var s = a[q];
// if (s) {
// var t = $.trim(s.keyword), u = t.toLowerCase().indexOf(p.toLowerCase()), v =
// t;
// if (0 == u && (v = p + "<strong>" + t.substring(u + p.length) + "</strong>"),
// "string" != typeof s.cid || $.trim(s.cid).isEmpty()) {
// var w = "";
// k += 0 == s.amount ? b.replace(e, {id: o,title: s.keyword,keyword: v,amount:
// s.amount,suggest_pos: j}) : b.replace(f, {id: o,title: s.keyword,keyword:
// v,amount: s.amount,suggest_pos: j,style: w}), o++, j++
// } else {
// if (0 == n) {
// n = !0;
// var w = 'style="visibility:hidden"', x = 0;
// s.oamount && s.oamount > 0 && (x = s.oamount, w = ""), k += b.replace(h, {id:
// o,title: s.keyword,keyword: v,amount: x,suggest_pos: j,style: w}), o++, j++
// }
...
// continue;
// var z = "item1", A = b.replace(g, {id: o,title: s.keyword,cid: s.cid,cLevel:
// s.level,className: z,cname: s.cname,amount: s.amount,suggest_pos: j - 1});
// k = b.replace(k, {categorys: A}), o++
// }
// }
// }
// c.length = o, d = b.replace(k, {categorys: ""}), "" != d ? (d += '<li
// class="close" onclick="$o.hideTip()">\u5173\u95ed</li>', m.html(d).show(),
// m.find('[id^="d_"]').bind("mouseleave", function() {
// c.mouseleave($(this))
...
// var b = this;
// a = a ? a : window.event, window.event ? (a.cancelBubble = !0, a.returnValue
// = !1) : (a.stopPropagation(), a.preventDefault());
// var c = $(a.srcElement ? a.srcElement : a.target), d =
// c.parent().parent().attr("title");
// $.ajax({url: "http://search.jd.com/suggest.php?op=del&callback=?&key=" +
// encodeURIComponent(d),dataType: "jsonp",scriptCharset: "utf-8",beforeSend:
// function() {
